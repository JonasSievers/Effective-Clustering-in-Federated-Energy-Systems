...
from CDCC.bilstm import BiLSTM

class CDCC(nn.Module):
    def __init__(self, configs):
        super(CDCC, self).__init__()
        # Time Encoder
        self.time_encoder=BiLSTM(configs.input_size, configs.hidden_size, configs.num_layers, configs.output_size,configs.dropout_rate)
        # Frequency Encoder
        ...
        self.cluster_projector_t = nn.Sequential(
            ...
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Linear(256, configs.cluster_size),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, x_in, x_in_aug,mode='t'):
        ...
        if mode == 'f':
            ...
            z_i_x = normalize(self.instance_projector_f(h_x), dim=1)  # z_i(batch*class_num)
            z_c_x = self.cluster_projector_f(h_x)

            z_i_aug = normalize(self.instance_projector_f(h_aug),
                                dim=1)  # z_i(batch*class_num)
