...
def DataTransform_T(data, model_params):
    ...
    li = np.random.randint(0, 2, size=[data.shape[0]])
    li_onehot = one_hot_encoding(li)
    zero_matrix = torch.zeros_like(data)

    mask = li_onehot[:,0].to(torch.bool).unsqueeze(-1).unsqueeze(-1)
    aug_1 = torch.where(mask, aug_1, zero_matrix)

    mask = li_onehot[:,1].to(torch.bool).unsqueeze(-1).unsqueeze(-1)
    aug_2 = torch.where(mask, aug_2, zero_matrix)
...
def DataTransform_F(sample, model_params):
    ...
    li_onehot = one_hot_encoding(li)
    zero_matrix = torch.zeros_like(sample)
